...
export default function PublicProfileAlbums() {
  ...
  usePageTitle(username ? `@${username} Albums` : 'Public Album Reviews')
  ...
  if (query.isError) return <ErrorState message={(query.error as any)?.response?.data?.error || 'Album reviews not found.'} onRetry={() => query.refetch()} />
  return (
