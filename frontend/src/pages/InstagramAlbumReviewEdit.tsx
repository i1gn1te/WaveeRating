...
import { getAlbumReview, updateAlbumReview } from '../lib/instagramReviewsApi'
...
interface SavedAlbumReviewEditData {
  id: string
  albumTitle: string
...
export default function InstagramAlbumReviewEdit() {
  const { id } = useParams()
  ...
  usePageTitle('Edit Album Review')

  const reviewQuery = useQuery<SavedAlbumReviewEditData>({
    queryKey: ['instagram-profile-album-edit', id],
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) => {
      setError(err?.response?.data?.error || 'Failed to update album review.')
      setMessage(null)
    },
  })
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <nav className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <Link to={`/library/albums/${review.id}`} className="inline-flex items-center gap-2 text-sm text-gray-400 transition hover:text-white">
            <ArrowLeft className="h-4 w-4" />
            Back to album review
          </Link>
        ...
        <section className="mb-8">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-pink-300">Edit album review</p>
    ...
    <label className="block">
      ...
      <div className="mt-2 flex items-center gap-3">
        <input
          ...
          className="w-32 rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 text-white outline-none transition focus:border-pink-400"
        />
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-60 ${
        primary ? 'bg-pink-500 text-white hover:bg-pink-400' : 'border border-gray-700 text-gray-100 hover:border-pink-300'
      }`}
    >
